...
#---------------------------------
# If argument to this script contains 'ecflow' (e.g. decflow01 or cecflow02), 
...
elif [ $arg1 = d1 -o $arg1 = d2 -o $arg1 = c1 -o $arg1 = c2 ]; then
  p1=`echo $arg1 | cut -c 1-1`
  s1=`echo $arg1 | cut -c 2-2`
  monitored_ecflow=${p1}ecflow0${s1}
else
  echo 'Unrecognized ecf server option. Exit.'
  exit 
fi
